...
# stat : how to compute the test statistic
IWT_Block_covariance <- function(data,blocks,B=1000, stat = "sum"){  
  ...
  if (stat == "sum") {
    ...
    stat.test = function(block1,block2){
      Rhohat = cor(block1,block2,method='pearson')
      Rohat.std = Rhohat^2/(1-Rhohat^2)
      return(max(Rohat.std)) # as for now, we use just standardized squared correlation as test statistic
    }  
  } else if (stat == "mean") {
    stat.test = function(block1,block2){
      Rhohat = cor(block1,block2,method='pearson')
      Rohat.std = Rhohat^2/(1-Rhohat^2)
      return(mean(Rohat.std)) # as for now, we use just standardized squared correlation as test statistic
    }  
  }
