...
blocks_loop=function(ix,nblocks,data,blocks,B,corrected.pval,pval_array,estimation=estimation){
...
IWT_Block_precision <- function(data,blocks,B=1000,nworkers=1,estimation='SCAD'){  # estimation method is either SCAD or LM
...
#  }else{
 #   registerDoSEQ()
  #}
  cl <- makeCluster(nworkers)
  result <- clusterMap(cl, blocks_loop, 2:nblocks,
